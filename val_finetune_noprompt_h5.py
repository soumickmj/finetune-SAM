...
import matplotlib.pyplot as plt
import copy
from utils.datasetH5 import Public_H5dataset
...
def main(args,test_image_list):
    ...
    test_dataset = Public_H5dataset(args,args.h5_path, phase='test',targets=args.targets,if_prompt=False,delete_empty_masks=False) #for testing, we will consider all the samples (including empty masks)
    ...
    with h5py.File(os.path.join(args.seg_save_dir, args.proc_tag, 'seg.h5'), 'w') as f:
        for i,data in enumerate(tqdm(testloader)):
            ...
            # msks = msks.to('cuda')
            img_name_list.append(data['ds_path'][0])
            ...
            if bool(args.seg_save_dir):
                ...
                if args.post_process_mask:
                    pred_mask = post_process_mask(pred_mask, fill_holes=args.post_process_fillholes, keep_largest_component=args.post_process_largestsegment)
                pred_mask = np.expand_dims(np.expand_dims(np.expand_dims(pred_mask, axis=0), axis=0), axis=0) #add channel, time and slice dim
                grp = f.require_group(os.path.dirname(data['ds_path'][0]))
                dset = grp.create_dataset(os.path.basename(data['ds_path'][0]), data=pred_mask)
                dset.attrs["min_val"] = 0
                dset.attrs["max_val"] = pred_mask.max()
    ...
    if args.store_emb:
        with h5py.File(os.path.join(save_folder,'embs.h5'), 'w') as hf:
            for item in emb_storage:
                group = hf.create_group(item["ds_path"])
