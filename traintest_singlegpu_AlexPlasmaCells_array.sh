...
# Parse the configuration line (format: expID|dsID|dsTag|splitTag|init_mode|peft_mode|targets|batch_size|num_workers|dsRoot|out_type|load_all_mask)
IFS='|' read -r expID dsID dsTag splitTag init_mode peft_mode targets batch_size num_workers dsRoot out_type load_all_mask <<< "$config_line"
...
train_iouhead="${train_iouhead:-False}"
...
srun /home/soumick.chatterjee/.local/bin/poetry run python SingleGPU_train_finetune_noprompt.py \
    --dir_checkpoint "$dsRoot/checkpoints/${out_type}" \
