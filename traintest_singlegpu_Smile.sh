...
#SBATCH --chdir=/group/glastonbury/soumick/SLURM/ftSAM_Smile
...
dsID="${dsID:-smile}"
dsTag="${dsTag:-synapse}"
dsRoot="${dsRoot:-/group/glastonbury/soumick/smile}"
splitTag="${splitTag:-N1}"
init_mode="${init_mode:-SAM}"
peft_mode="${peft_mode:-adapter}"
batch_size="${batch_size:-3}"
num_workers="${num_workers:-3}"

prenorm_type="${prenorm_type:-window}"
prenorm_window_min_percentile="${prenorm_window_min_percentile:-1}"
prenorm_window_max_percentile="${prenorm_window_max_percentile:-99}"

# Fixed params
arch="vit_b"

dataset_name="Smile_${dsTag}"
train_img_list="${dsRoot}/CSVs/train_slices_${splitTag}.csv"
val_img_list="${dsRoot}/CSVs/val_slices_${splitTag}.csv"
test_img_list="${dsRoot}/CSVs/test_slices_${splitTag}.csv"

seg_save_dir="${dsRoot}/ftSAM/segmentations"
...
srun poetry run python SingleGPU_train_finetune_noprompt.py \
    ...
    --w "$num_workers" \
    --run_tag "$run_tag" \
    --dir_checkpoint "$dsRoot/ftSAM/checkpoints" 
...
srun poetry run python val_finetune_noprompt.py \
    ...
    --seg_save_dir "${seg_save_dir}/prefinetune_${run_tag}" \
    --run_tag "$run_tag" \
    --dir_checkpoint "$dsRoot/ftSAM/checkpoints"
...
srun poetry run python val_finetune_noprompt.py \
    ...
    --test_img_list "$test_img_list" \
    --run_tag "$run_tag" \
    --seg_save_dir "${seg_save_dir}/${run_tag}" \
    --dir_checkpoint "$dsRoot/ftSAM/checkpoints"
